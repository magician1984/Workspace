...
import com.auo.dvr.launcher.observer.PollingFileObserver
...
internal class DvrLauncher(
    ...
    companion object {
        ...
        private const val LOCK_FILE_EXTENSION = "lock"
    ...
    override var onRecordUpdateListener: IDvrLauncher.OnRecordUpdateListener? = null
    ...
    private val mFileObserver: IFileObserver =
        object : PollingFileObserver(mSharedPartitionFolder, 1000) {
            override fun onEvent(event: EventType, file: File) {
                Log.d("DvrLauncher", "onEvent: ${event}, ${file.name}")
                if (file.extension == LOCK_FILE_EXTENSION)
                    mQnxServerMonitor.onFileUpdate(event, file)
                else if (file.extension == WORKAROUND_FILE_EXTENSION || file.extension == EVENT_FILE_EXTENSION)
                    mWorkaround.onEvent(event, file)
            }
        }
    ...
    private fun workaround(eventType: EventType, file: File) {
        if (eventType != EventType.Create && eventType != EventType.Exist)
            return
