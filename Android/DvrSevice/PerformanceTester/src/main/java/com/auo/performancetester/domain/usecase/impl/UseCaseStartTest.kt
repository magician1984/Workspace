...
class UseCaseStartTest(private val testSource: IDataSource) : IUseCaseStartTest {
    override fun invoke(method: CloneMethod, allocateMode: FileAllocateMode, fileSize: FileSize, count: Int, forceWrite: Boolean) {
        testSource.startTest(TestCaseConfigure(method, allocateMode, fileSize,  count, forceWrite, 1024*1024))
    }
}