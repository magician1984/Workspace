...
internal class RecordListView : Presenter.IView {
    ...
    override fun Draw(
        ...
        HorizontalPager(state = pageState, beyondBoundsPageCount = pageState.pageCount) { page ->
            val subList = state.fileList.filter { it.location == CamLocation.entries[page] && if(state.isProtected) it.type == RecordType.Protected else true }
        ...
        LazyColumn(
            ...
            items(recordList) { record ->
                RecordItem(
                    ...
                    isSelected = record == selectedFile,
                    onIntent = onIntent
                )
            }
        }
